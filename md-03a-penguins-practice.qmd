...
5.  What data types are contained in the variables? nominal, continuous, categorical (Reminder: <https://ds4owd-001.github.io/website/slides/lec-02-visualisation.html#/types-of-variables>)
...
```{r}
penguins |> 
  filter(sex == "female", bill_length_mm < 35)
```
...
```{r}
penguins |> 
  mutate(id = row_number(penguins$species)) |> 
  relocate(id)
```
...
penguins |> 
  ...
  #I'm not sure if this is the right way of doing it, filtering it twice
  group_by(species) |> 
  summarise(
